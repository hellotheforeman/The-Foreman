...
async function sendBriefingForBusiness(business) {
  ...
  if (!dueToday.length && !overdue.length && !staleQuotes.length) return false;
  ...
  if (meta?.last_briefing_hash === hash && daysSinceLastSend < REPEAT_DAYS) return false;
  ...
  cron.schedule('0 8 * * *', async () => {
    try {
      ...
      for (const business of active) {
        try {
          await sendBriefingForBusiness(business);
        } catch (err) {
...
module.exports = { start, sendBriefingForBusiness };
