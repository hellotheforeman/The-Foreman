...
function testTemplateRendering() {
  const business = { name: 'Boiler & Co' };
  const customer = { name: 'Mrs Patel', phone: '+447700900123' };
  const job = { id: 12, description: 'Boiler service', quote_items: 'Boiler service', quoted_amount: 85 };
  const invoice = { amount: 85, line_items: 'Boiler service', sent_at: new Date().toISOString() };
