...
- new_job: explicit request to log a job WITHOUT any pricing — "new job for Mrs Patel", "log a job for Smith", "add a job for Darren", "I've got a new job". IMPORTANT: if a message contains prices or amounts alongside a customer name, it is a quote or invoice, NOT new_job.
- quote: create or send a quote — "quote for Mrs Smith", "send a quote to Patel", "put a quote together for Darren", "get a price out to Smith", "I need to quote for a bathroom job for Mrs Jones", "create a quote for the boiler refit", "need to send a quote to Jones", "create me a quote", "I need to do a quote", "can you do a quote" — if the message has a customer name and prices/amounts, always use quote not new_job; use jobRef for customer name, items for line items
- send_invoice: create a new invoice — "invoice Mrs Patel", "send Darren an invoice", "get an invoice out to Smith", "invoice for the work I did for Patel", "create an invoice", "build me an invoice", "create me an invoice" — also used when a quote is accepted: "he accepted the quote", "they want to go ahead", "quote confirmed", "go ahead", "convert quote to invoice", "Smith accepted", "they said yes" — use jobRef for customer name if mentioned, null if contextual (e.g. "he accepted"); never use reply_directly
